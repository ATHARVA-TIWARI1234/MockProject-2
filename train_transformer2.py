...
# pip install -U "torch>=2.1" torchvision torchaudio   # ensure MPS-enabled PyTorch installed
# -----------------------------
...
# -----------------------------
# BGE-small Embeddings (Apple GPU MPS)
# -----------------------------
print("⚡ Generating BGE-small embeddings on MPS if available...")
...
resume_embeddings = bge_model.encode(resume_texts, convert_to_numpy=True, batch_size=16, show_progress_bar=True)
jd_embeddings = bge_model.encode(jd_texts, convert_to_numpy=True, batch_size=16, show_progress_bar=True)
...
with open(os.path.join(MODEL_DIR, "scaler_bge_extra.pkl"), "wb") as f:
...
with torch.no_grad():
    preds = transformer_model(X_test_tensor)
    preds_cpu = preds.cpu().numpy()
    y_test_cpu = y_test_tensor.cpu().numpy()
    y_pred_label = (preds_cpu > 0.5).astype(int)

print("\n✅ BGE-small + Lightweight Transformer + Extra Features:")
...
# Save model & scaler
# -----------------------------
torch.save(transformer_model.state_dict(), os.path.join(MODEL_DIR, "model_bge_transformer_extra_mps.pt"))
